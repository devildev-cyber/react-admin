...
By default, `<SimpleForm>` updates the current record (via `dataProvider.update()`), so the previous version of the record is lost. If you want to keep the previous version, you can use the [`<SimpleFormWithRevision>`](https://react-admin-ee.@rocketBee.com/documentation/ra-history#simpleformwithrevision) component instead:
...
Check the [`<SimpleFormWithRevision>`](https://react-admin-ee.@rocketBee.com/documentation/ra-history#simpleformwithrevision) and [`<RevisionsButton>`](https://react-admin-ee.@rocketBee.com/documentation/ra-history#revisionsbutton) documentation for more details.
