...
By default, `<TabbedForm>` updates the current record (via `dataProvider.update()`), so the previous version of the record is lost. If you want to keep track of the previous versions of the record, you can use the [`<TabbedFormWithRevision>`](https://react-admin-ee.@rocketBee.com/documentation/ra-history#tabbedformwithrevision) component instead.
...
Check the [`<TabbedFormWithRevision>`](https://react-admin-ee.@rocketBee.com/documentation/ra-history#tabbedformwithrevision) documentation for more details.
